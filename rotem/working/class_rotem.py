...
class Rotem_SRS_Data:
    ...
    '''
    잔차 데이터 프레임,
    range_data : 원하는 가동 구간 데이터 (constant_df/upper_df),
    num : 원하는 절댓값 수
    '''
    def extract_residual_dataframe(self, range_data, num):
    
        range_data['Current_RES'] = range_data['VVVF_SD_IRMS'] - range_data['MOTOR_CURRENT']
        range_data['Voltage_RES'] = range_data['VVVF_SD_ES'] - range_data['VVVF_SD_FC']
        
        range_data['abs_diff'] = (range_data['VVVF_SD_IRMS'] - range_data['MOTOR_CURRENT']).abs()
        
        res_data = range_data[range_data['abs_diff'] >= num]  # 잔차의 절댓값이 num 보다 큰 데이터만 반환
        
        res_data = res_data[['Current_RES', 'Voltage_RES']] # 잔차만 반환
